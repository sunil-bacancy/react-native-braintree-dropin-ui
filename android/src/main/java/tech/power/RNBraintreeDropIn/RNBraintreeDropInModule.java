...
public class RNBraintreeDropInModule extends ReactContextBaseJavaModule {
  ...
  public void show(final ReadableMap options, final Promise promise) {
    ...
    if (options.hasKey("threeDSecure")) {
      ...
      if (!threeDSecureOptions.hasKey("amount")) {
        promise.reject(
          "NO_3DS_AMOUNT",
          "You must provide an amount for 3D Secure"
        );
        return;
      }
    ...
    currentActivity.startActivityForResult(
      dropInRequest.getIntent(currentActivity),
      DROP_IN_REQUEST
    );
  }
  ...
  private final ActivityEventListener mActivityListener = new BaseActivityEventListener() {
    ...
    public void onActivityResult(
      ...
      if (resultCode == Activity.RESULT_OK) {
        ...
        if (
          isVerifyingThreeDSecure && paymentMethodNonce instanceof CardNonce
        ) {
        ...
        Exception exception = (Exception) data.getSerializableExtra(
  ...
  private final void resolvePayment(
